...
json 형태의 key, value 값을 통해서 관리되는데 저장되는 값은 Bucket Path를 가리키는 Key값과 해당 Path의 값을 포함합니다.
...
gaia에서 key값은 실제 s3 Bucket을 찾아가는데 어떠한 영향도 미치지 않기 때문에 명명에 자유롭습니다. 또한 gaia는 S3 Bucket Path가 날짜 단위로
